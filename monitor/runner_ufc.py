...
def run_ufc() -> None:
    ...
    try:
        ...
        raw_odds = clients.get_mma_fiat_data()
        raw_poly = clients.get_mma_polymarket_events()
        ...
        for game in raw_odds:
            ...
            if k not in fiat_games:
                fiat_games[k] = {
                    ...
                    "sport_key": game.get("sport_key", "mma"),
                    "bookies": [],
                }
        ...
        for gk, x in fiat_games.items():
                    ...
                    for pt, t1_odds in b1.get("totals", {}).items():
                        ...
                        if o1_over and o2_under:
                            ...
                            log_raw_detection_to_csv(
                                category="UFC",
                                opportunity_type="FIAT_FIAT",
                                market=f"Total Rounds {pt}",
                                home_team=x["home"],
                                away_team=x["away"],
                                game_date=format_to_local(x["time"]),
                                bookmaker=b1["name"],
                                selection="Over",
                                odds_decimal=float(o1_over),
                                opposite_selection="Under",
                                opposite_odds_decimal=float(o2_under),
                                implied_total=float(imp),
                                roi_percent=roi,
                                passed_liquidity_filter="",
                                passed_roi_filter=passed_roi_filter,
                                was_profitable=was_profitable,
                                reject_reason="" if was_profitable else "Not profitable or outside ROI filter",
                                notes=f"Compared {b1['name']} Over vs {b2['name']} Under",
                                run_id=run_id,
                            )
                        ...
                        if o1_under and o2_over:
                            ...
                            log_raw_detection_to_csv(
                                ...
                                bookmaker=b1["name"],
                                selection="Under",
                                odds_decimal=float(o1_under),
                                opposite_selection="Over",
                                opposite_odds_decimal=float(o2_over),
                                implied_total=float(imp),
                                roi_percent=roi,
                                passed_liquidity_filter="",
                                passed_roi_filter=passed_roi_filter,
                                was_profitable=was_profitable,
                                reject_reason="" if was_profitable else "Not profitable or outside ROI filter",
                                notes=f"Compared {b1['name']} Under vs {b2['name']} Over",
                                run_id=run_id,
                            )
            ...
            for b in x["bookies"]:
                for m in target.get("markets", []):
                            ...
                            if f_odds:
                                ...
                                if f_opp:
                                    ...
                                    log_raw_detection_to_csv(
                                        ...
                                        notes="UFC/MMA moneyline raw check",
                                        run_id=run_id,
                                    )

                                    logger.info(
                                        f"   [ML] {b['name']:<12} | {t_nm[:10]:<10} | {b['name']} Opp: {float(f_opp):<5} | Poly Ask: {poly_price:<5} | Status: {'✅' if hedge.passes_liquidity_filter else '❌ ' + str(hedge.reject_reason)}"
                                    )
                    ...
                    elif mt == "round_over_under_match" or "round" in question or "over" in question or "under" in question:
                        ...
                        for idx, out_lbl in enumerate(outs):
                            ...
                            if f_opp:
                                ...
                                log_raw_detection_to_csv(
                                    ...
                                    was_profitable=was_profitable,
                                    reject_reason="" if was_profitable else str(hedge.reject_reason or "Outside ROI filter"),
                                    notes="UFC/MMA total rounds raw check",
                                    run_id=run_id,
                                )

                                logger.info(
                                    f"   [TOT] {b['name']:<11} | {poly_side[:10]:<10} | {b['name']} Opp: {float(f_opp):<5} | Poly Ask: {poly_price:<5} | Status: {'✅' if hedge.passes_liquidity_filter else '❌ ' + str(hedge.reject_reason)}"
                                )
